...
func New(eth Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	...
	if tribe, ok := miner.engine.(*tribe.Tribe); ok {
		...
		go func() {
			...
			tma := <-rtn
			log.Info("miner get miner address", "addr", tma.Hex())
			go miner.Start(tma)
...
func (self *Miner) Start(coinbase common.Address) {
	...
	if _tribe, ok := self.engine.(*tribe.Tribe); ok && self.eth.BlockChain().CurrentBlock().NumberU64() > 3 {
		i := 0
		for {
			m := _tribe.Status.GetMinerAddress()
			s, err := self.worker.chain.State()
			log.Debug("<<MinerStart>> loop_start", "i", i, "num", self.eth.BlockChain().CurrentBlock().Number(), "addr", m.Hex(), "level", _tribe.Status.SignerLevel)
			if err != nil {
