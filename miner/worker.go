...
func (self *worker) commitNewWork() {
	...
	if _tribe, ok := self.engine.(*tribe.Tribe); ok {
		chiefTxs := _tribe.GetChiefUpdateTx(self.chain, header, work.state)
		//log.Debug("chiefTxs","txs",chiefTxs)
		if chiefTxs != nil && len(chiefTxs) > 0 {
			pending[_tribe.Status.GetMinerAddress()] = chiefTxs
			//} else if params.IsSIP100Block(header.Number) && _tribe.Status.SignerLevel != tribe.LevelSigner {
			//log.Debug(" 🙏 Is not a signer , skip work .", "num", header.Number, "signerLevel", _tribe.Status.SignerLevel)
		}
	}
