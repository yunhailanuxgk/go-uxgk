...
func (self *TribeStatus) ValidateBlock(state *state.StateDB, parent, block *types.Block, validateSigner bool) error {
	if parent.Number().Int64() == 0 {
		ga := common.BytesToAddress(parent.Extra()[32 : 32+20])
		if block.Coinbase() != ga {
			err := fmt.Errorf("error_genesis_signer, want %s , found %s", ga.Hex(), block.Coinbase().Hex())
			log.Warn("⚠️ Validate block fail , error signer", "err", err)
			return err
		}
	}
	...
	for i, tx := range block.Transactions() {
		...
		*/
		if tx.To() != nil && params.IsChiefAddressOnBlock(block.Number(), *tx.To()) && params.IsChiefUpdate(tx.Data()) {
			...
			//verify volunteer
			if state != nil {
				if params.IsSIP100Block(new(big.Int).Sub(header.Number, big.NewInt(1))) {
