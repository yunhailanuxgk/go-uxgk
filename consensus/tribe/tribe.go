// Copyright 2018 The UXGK Authors
package tribe
...
import (
	...
	"github.com/yunhailanuxgk/go-uxgk/common/math"
	...
	"github.com/yunhailanuxgk/go-uxgk/crypto/sha3"
...
func (t *Tribe) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	log.Debug("Prepare", "num", header.Number)
...
func (t *Tribe) GetChiefUpdateTx(chain consensus.ChainReader, header *types.Header, state *state.StateDB) []*types.Transaction {
	//txs := t.setChiefConfig(chain, header, state)
	...
	nonce := uint64(len(txs)) + state.GetNonce(t.Status.GetMinerAddress())
	...
	if err != nil {
		panic(fmt.Sprintf("sign tx:%s\r\n", err))
	}
	return append(txs, signedTx)
}
...
func (t *Tribe) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	if len(block.Transactions()) == 0 && block.Number().Int64() > CHIEF_NUMBER {
