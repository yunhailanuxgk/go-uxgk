...
import (
	...
	"github.com/yunhailanuxgk/go-uxgk/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geth is "uxgk". If no
...
func (c *Config) NodeName() string {
	...
	if name == "uxgk" || name == "uxgk-testnet" {
		name = "uxgk"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "uxgk" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "uxgk" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "uxgk" {
...
func (c *Config) loadECDSAWithPwd() (*ecdsa.PrivateKey, error) {
	...
	overFunc := func() {
		fmt.Println("=================================================")
		fmt.Println(`> Must be first execute "uxgk security --unlock" `)
		fmt.Println("=================================================")
		os.Exit(-1)
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "UXGK-keystore")
		ephemeral = keydir
	}
